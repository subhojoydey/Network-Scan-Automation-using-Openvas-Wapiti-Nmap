...
openvas-start
echo "1. Do you want to use previous target?\n2. Do you want to scan argument target?\n3.Do you want to use previous task?"
read choice
...
if [ $choice -eq 1 ]
 ...
 then
	...
	task_id=$( omp --username subho --password zaq12wsx --get-tasks | 
		awk -vX="$name_of_target" '{ if( $3==X ){ print $1 }}' )
...
report_id=$( omp --username subho --password zaq12wsx --get-tasks "$task_id" | awk 'NR==2 {print $1}' )
...
temp_client_var=$( ls /var/lib/NetworkManager | lease )
